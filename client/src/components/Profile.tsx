...
import { User, Settings, Trash2, Shield, Coins, Droplets, AlertTriangle, Check, X, LogOut, RefreshCw } from 'lucide-react';
...
const Profile: React.FC = () => {
    const { 
        user, mainCharacter, party, gold, bloodRations, currentFloor, biome, 
            ...
            <div className="flex bg-white/5 p-1.5 rounded-xl gap-2 border border-white/5">
                <button
                    ...
                    className={`flex-1 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                        activeTab === 'profile' ? 'bg-primary-color text-white' : 'text-muted hover:text-white'
                    }`}
                >
                    Profile
                </button>
                <button
                    ...
                    className={`flex-1 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                        activeTab === 'settings' ? 'bg-primary-color text-white' : 'text-muted hover:text-white'
                    }`}
                >
                    Settings
                </button>
            ...
            {activeTab === 'profile' && (
                <div className="space-y-6">
                    ...
                    {mainCharacter && (
                        <div className="glass p-6 rounded-3xl border border-crimson/20 space-y-5">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="badge bg-primary-color/10 text-primary-color border-primary-color/20">
                    ...
                    <div className="glass p-6 rounded-3xl border border-white/5 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="flex items-center gap-3 p-4 bg-black/30 rounded-xl border border-white/5">
                                ...
                                <div>
                                    <div className="text-lg font-black">{gold.toLocaleString()}</div>
                                    <div className="text-[8px] text-muted uppercase">Gold</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-4 bg-black/30 rounded-xl border border-white/5">
                                <Droplets size={18} className="text-red-500" />
                                <div>
                                    <div className="text-lg font-black">{Math.floor(bloodRations)}</div>
                                    <div className="text-[8px] text-muted uppercase">Blood Rations</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-4 bg-black/30 rounded-xl border border-white/5">
                                <Shield size={18} className="text-primary-color" />
                                <div>
                                    <div className="text-lg font-black">{fullParty.length}/4</div>
                                    <div className="text-[8px] text-muted uppercase">Party Size</div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-4 bg-black/30 rounded-xl border border-white/5">
                                <User size={18} className="text-secondary-color" />
                                <div>
                                    <div className="text-lg font-black">{councilMembers.length}</div>
                                    <div className="text-[8px] text-muted uppercase">Council</div>
                                </div>
                            </div>
                        </div>

                        <div className="p-4 bg-primary-color/5 rounded-xl border border-primary-color/20">
                            <div className="flex items-center gap-3">
                                <div className={`w-3 h-3 rounded-full ${isResonatorActive ? 'bg-primary-color animate-pulse' : 'bg-white/20'}`} />
                                <div className="flex-1">
                                    <div className="text-sm font-black">Resonator {isResonatorActive ? 'Active' : 'Inactive'}</div>
                                    <div className="text-[9px] text-muted">Mastery Level {resonatorMastery + 1} • {isResonatorActive ? '+10%' : 'No Bonus'}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="glass p-6 rounded-3xl border border-white/5 space-y-3">
                        <h3 className="text-sm font-black uppercase tracking-wider text-white/60 mb-4">Account Actions</h3>
                        
                        <button
                            onClick={handleForceSave}
                            className="w-full py-4 bg-primary-color/10 border border-primary-color/20 rounded-2xl text-[10px] font-black uppercase tracking-widest text-primary-color flex items-center justify-center gap-3 active:scale-95 transition-transform"
                        >
                            <RefreshCw size={16} />
                            Force Save Progress
                        </button>
                        ...
                        <button
                            onClick={logout}
                            className="w-full py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-muted flex items-center justify-center gap-3 active:scale-95 transition-transform hover:bg-white/10"
                        >
                            <LogOut size={16} />
                            Logout
                        </button>
                    ...
                    <div className="glass p-6 rounded-3xl border border-danger-color/20 space-y-4">
                        ...
                        <p className="text-xs text-muted leading-relaxed">
                            Deleting your account is permanent. All progress, characters, items, and achievements will be lost forever.
                        </p>

                        {!showDeleteConfirm ? (
                            <button
                                onClick={() => setShowDeleteConfirm(true)}
                                className="w-full py-4 bg-danger-color/10 border border-danger-color/20 rounded-2xl text-[10px] font-black uppercase tracking-widest text-danger-color flex items-center justify-center gap-3 active:scale-95 transition-transform"
                            >
                                <Trash2 size={16} />
                                Delete Account
                            </button>
                            ...
                            <div className="space-y-3">
                                <div className="p-4 bg-danger-color/5 rounded-xl border border-danger-color/20">
                                    <p className="text-xs text-danger-color font-bold mb-2">This action cannot be undone!</p>
                                    <p className="text-[10px] text-muted">Type <span className="text-white font-black">DELETE</span> to confirm:</p>
                                </div>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-3 bg-black/40 border border-danger-color/30 rounded-xl text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-danger-color"
                                />
                                <div className="flex gap-3">
                                    <button
                                        onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(''); }}
                                        className="flex-1 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-muted flex items-center justify-center gap-2"
                                    >
                                        <X size={14} />
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 py-3 bg-danger-color text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-50"
                                    >
                                        {deleting ? 'Deleting...' : (
                                            <>
                                                <Check size={14} />
                ...
                <div className="space-y-4">
                    <div className="glass p-6 rounded-3xl border border-white/5 space-y-5">
                        ...
                        {[
                            ...
                            { key: 'compactMode', label: 'Compact Mode', desc: 'Show more content with smaller UI elements' }
                        ].map(setting => (
                            <div key={setting.key} className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`w-14 h-7 rounded-full transition-all relative ${
                                        settings[setting.key as keyof typeof settings] ? 'bg-primary-color' : 'bg-white/10'
                                    }`}
                                >
                                    <div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-lg transition-all ${
                                        settings[setting.key as keyof typeof settings] ? 'left-8' : 'left-1'
                                    }`} />
                                </button>
                            </div>
                        ))}
                    ...
                    <div className="glass p-6 rounded-3xl border border-white/5 space-y-4">
                        ...
                        <div className="space-y-3 text-xs">
                            ...
                            <div className="flex justify-between p-3 bg-black/30 rounded-xl">
                                <span className="text-muted">Biome</span>
