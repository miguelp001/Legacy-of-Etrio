...
const BASE_STATS: Record<BaseClass, Stats> = {
...
const GROWTH_RATES: Record<BaseClass, Stats> = {
...
const SUBCLASS_GROWTH_MODIFIERS: Record<SubClass, Partial<Stats>> = {
...
export class StatCalculator {
    static calculateStats(level: number, baseClass: BaseClass, generation: number, subClass?: SubClass): Stats {
        ...
        if (level >= 20 && subClass) {
            ...
            if (mod.strength) stats.strength += (growth.strength * mod.strength * subLevel);
            if (mod.agility) stats.agility += (growth.agility * mod.agility * subLevel);
            if (mod.intelligence) stats.intelligence += (growth.intelligence * mod.intelligence * subLevel);
            if (mod.vitality) stats.vitality += (growth.vitality * mod.vitality * subLevel);
            if (mod.spirit) stats.spirit += (growth.spirit * mod.spirit * subLevel);
            if (mod.luck) stats.luck += (growth.luck * mod.luck * subLevel);
        }
